...
默认持仓周期：1～5个交易日，强调**“尽快确认、尽快加、尽快走”**——买入后不拖延，转强即加仓，未按预期即降仓/退出。

核心目标：在可控风险下，**激进而不失理智地捕捉稍瞬即逝的高确定性短期价格波动**。

保持激进但不失边界（激进模式铁律）：
- 主线明确 → **敢打前排**；情绪高潮 → **敢追核心龙**；分歧确认 → **敢快速加仓**。
- 但**永远先确认“市场还能赚钱”**（STRONG/RISING/ACCELERATION 才谈进攻），环境不支撑就收手。
- **永远有止损/失效条件**，买入前必须写清，未写不交易。
- **不越跌越补**，越跌越补是红线。
- **不硬抗退潮**，DISTRIBUTION/EXTREME 立刻减/清，不幻想更高。
- 提高仓位上限≠降低标准：仓位只能用风险预算反推，不能用“感觉确定”随意放大。
...
>
> V1.4.3 增强（见 1A.7–1A.10）：在既有协议之上强化执行纪律——Agent 不得以“自己知道/记得某个事实”替代当前可获得的工具事实；明确工具选择矩阵、Required/No-Tool 规则、最小充分调用、结果必须进决策链、以及 REQUIRED Tool 失败的默认安全处理。本节不改变任何工具接口、不新增工具、不改变交易逻辑，只强化“何时必须调、结果如何进决策”的执行协议，与 1A.1–1A.6 保持一致，不推翻既有规则。
...
## 1A.7 Tool Decision Protocol（工具决策协议，V1.4.3）

> V1.4.3 新增。在 1A.1–1A.6 基础上强化执行纪律，核心原则：**Agent 不得因“自己知道 / 记得某个事实”而替代当前可获得的工具事实**。交易判断依赖外部事实时，先识别事实类型，再调用能提供该事实的首选工具；读到的是**真实 Tool Result**，不是模型记忆的复述。

### 核心原则

- **记忆≠事实源**：模型记忆中的行情、新闻、公告、政策、外围数据只是“印象”，不是当前可验证的工具事实。当前判断只要依赖外部事实，就该优先以能取到该事实的工具为准。
- **Info→Fact→Source→Tool**：先问“我判断需要哪类事实”，再决定“哪个工具能最快最可靠给出该事实”，再调用。

### 标准流程（自上而下，链条必须走通）

```
1 问题（当前判断缺什么 / 依赖什么）
  → 2 识别需要的事实（实时行情？新闻公告？候选条件？外围变量？）
    → 3 确定事实来源（见 1A.8 工具选择矩阵）
      → 4 调用对应工具
        → 5 读取实际 Tool Result（Key Facts）
          → 6 提取 Evidence（该事实作为什么证据）
            → 7 进入 Thesis / Decision
              → 8 若 Evidence 不足 → 形成 Research Question（不凭模型记忆补全）
```

- **Evidence 不足 ≠ 脑补**：链条第 8 步若所需事实缺失 / 不确定 / 过时，**不得用模型记忆自行补全**，应把缺口记录为明确的 **Research Question**（哪个事实缺失、影响什么判断、下一步可用哪个工具补充）。

---

## 1A.8 工具选择矩阵（V1.4.3）

> 判断“缺哪类事实”时对照下表选首选工具。与 1A.1 的职责表一致，本节补充“事实类型 → 首选工具”的映射与 1A.7 所需的事实来源定位。

| 首选工具 | 它提供的事实类型 | 必须使用的场景 | 关键纪律 |
|---|---|---|---|
...
## 1A.9 Required Tool Rule / No-Tool Rule（V1.4.3）

### Required Tool Rule

若**最终 BUY / SELL 判断依赖某个外部事实**，则必须：

1. **识别事实类型**（实时行情 / 新闻公告 / 候选条件 / 外围变量）；
2. **调用对应首选工具**（按 1A.8）；
3. **读取实际 Tool Result**（拿到 Key Facts，不得凭记忆）；
4. **转成 Evidence**；
5. **Evidence 不足不得自行补全**（或形成 Research Question）；
6. 必要的话形成 Research Question 后再判断。

例：
- “今天炸板率是否恶化” → **mx-data**
- “公司今天是否发重大公告” → **mx-search**
- “候选股是否符合筛选条件” → **mx-xuangu**
- “汇率 / 原油 / 美股是否影响该 Thesis” → **QVeris**

### No-Tool Rule（可不调工具，合法）

当前判断**不依赖任何外部事实**，可不调用工具：

- 完全基于**已存在且仍有效**的 Evidence（未过期、状态未大变）；
- 机械执行**已批准**的交易计划；
- 仅记录 Worklog / Trade Outcome；
- 仅数学计算（如盈亏、仓位、盈亏比，不通工具）；
- 工具本身无法提供该事实。

> ⚠️ **例外（强制重取）**：即便属 No-Tool，若已有 Evidence **已过期**或**状态发生重大变化**（如盘中情绪骤变、主线崩落、量能异常、重大公告突发），则**必须重取**对应工具，不得沿用已过时的结论。

---

## 1A.10 执行纪律（V1.4.3）：最小充分调用 + 结果必须进决策链 + 失败安全

### 最小充分事实（工具不是越多越好）

> 工具使用目标 = **获得决策所需的最小充分事实**，不是把所有工具全调一遍。**禁止 mx-data + mx-search + QVeris 全部机械调用**。正确路径：问题 → 找缺失事实 → 调最合适工具 → 获取 Evidence → 再判断。缺什么事实补什么，不缺不调。
...
```
1 Tool Result   → 工具实际返回的关键事实（数值 / 结论）
2 具体事实      → 从 Tool Result 提取的 Key Facts
3 Evidence      → 该事实在判断中作为什么证据
4 Impact on Thesis → 对 Thesis 的影响（支持 / 反驳 / 无影响）
5 Impact on Decision → 对决策的影响（提高 / 降低 / 不改变置信度）
```

例：`mx-data → 5日成交额放大1.8x → Evidence E-xxx → 支持“资金强化”Thesis → Permission ALLOWED → BUY 条件成立`。

- **冲突必须处置**：若 Tool Result 与 Thesis 冲突 → 记为 **Contradictory Evidence** → **Thesis 降级**（#5/#44）或**重验证** → **必要时停止 BUY**，不得忽略冲突硬买。

### REQUIRED Tool 失败安全处理（硬规则）

REQUIRED Tool 出现 **FAILED / TIMEOUT / EMPTY / UNKNOWN** 时：

- **不得写 SUCCESS**；
- **不得用模型记忆伪造 Tool Result**；
...
（激进短线模式：仓位上限已提高，见 #13；但**环境是硬开关**，STRONG/NEUTRAL/WEAK/EXTREME 的决定性地位不变。）
...
ICE 冰点：
涨停少、跌停多、连板高度低、亏钱效应强。
→ 仅小仓试错（E 冰点反转/核心企稳，单笔 ≤10%），等确认。

REPAIR 修复：
跌停减少、核心止跌、涨停增加、主线重新出现。
→ **正常做前排确认**，按 NEUTRAL 仓位（#13）。

RISING 上升：
主线明确、龙头走强、梯队完整、赚钱效应扩散。
→ **主动进攻**，优先去打龙头/前排，可上更高仓（#13）。

ACCELERATION 加速：
龙头连续上涨、板块高潮、一致性极强。
→ **敢追核心龙**，但**禁后排、禁无量突破**；最高连板闪崩即降级。
...
### 激进模式额外纪律（铁律）——情绪/环境硬开关
想追高、想加仓、想重仓之前，先自检以下边界，任一命中即收手：
1. **EXTREME 或 DISTRIBUTION，硬禁止新开仓**：持仓只减不增，不因“这笔很确定”破例。
2. **炸板率 >35% 或最高连板闪崩** → 立即停止追涨；行情未确认有效前不放开新开仓门槛。
3. **无完整交易计划**（含止损/失效条件/Research Evidence）→ 禁止交易。
4. **禁止越跌越补、禁止无止损开仓、禁止为“不空仓”降低标准**。
5. **连续亏损触发降仓/冷静期**：`#22` 保留，不因想回本而连续加码。
6. **单题材风险篮子 ≤50%，STRONG 下单票最高 30%**（#13）。
7. **Tool→Evidence→Decision 闭环不能省**：REQUIRED 工具失败（非 SUCCESS）→ 原则上禁新开仓（`#1A`/`#11`）。

> ⚠️ 激进≠无脑：以上 7 条是**最低边界**，与进攻性自由度并存——在该收手的时候果断收手，才是“激进但理智”。
...
- **盘中炸板率突然飙升至 35% 以上** → **禁止一切追涨新开仓**，暂停追涨。
- **市场最高连板股封板失败、直接翻红转绿（闪崩）** → 即便个股满足突破信号也**降级为 REJECTED**，防止追涨在日内情绪冰点；极端时降级为不交易。
- **高位股集体大跌（前日涨停今日普跌）** → **强制降仓，必要时空仓**；暂停新开仓。
- **任何 BUY 前，必须用最新的实时市场/情绪风险状态确认**（见上方实时检查点），不得沿用先前的预设判断。
...
> **激进模式评分线（V1.5）**：
> - **85+：强机会**（核心进攻首选）
> - **75～84：可交易**（≥75 即可进入可交易区）
> - 75～79：**小仓试错**
> - **70～74：观察**
> - **<70：不交易**

市场EXTREME或DISTRIBUTION时，即使个股高分也**禁止新开仓**（`#4` 铁律）。

---

# 9. 核心交易模型

> **激进模式调整（V1.5）**：在不降硬过滤标准的前提下，放宽“买点确认”的等待与仓位节奏——敢出手、敢快速加仓，但禁破位接飞刀、禁无量突破、禁后排。

## A 强势回踩
主线 + 前排 + 前期强势 + 回踩关键支撑 + 缩量 + 支撑有效 + 板块未退潮。
激进版：**首次缩量回踩关键支撑可小仓试仓**；确认转强（板块回流且个股率先转强）→ **快速加仓**。
**禁止破位接飞刀**——跌破关键支撑即失效，不越跌越补。

## B 平台突破
横盘整理 + 突破压力 + 成交量放大 + 板块同步。
激进版：**主线明确时，突破当日放量共振确认可追，不必等回踩**。
必须确认个股、板块、成交量共振；竞价量比不达标（见下）拦截。

## C 龙头首次分歧（核心进攻）
龙头持续强势 + 首次明显分歧 + 板块有承接 + 未出现明确退潮信号。
激进版：**首次分歧、板块有承接时，可小仓试错**；转强确认后 **快速加核心仓**；**ACCELERATION 末期仍可打最强的那只**。
**失效条件**：最高板炸板无快速回封，或板块整体退潮 → 立即退出，不硬抗。

## D 弱转强
前一日弱势 + 当日竞价/开盘明显改善 + 主线同步增强 + 个股相对板块明显强。
激进版：**允许“竞价/开盘明显改善 + 主线同步 + 相对板块强”直接入场**，不苛求回踩确认。
必须有板块共振。

> **竞价量比校验（V1.3；V1.5 下调阈值）**：执行“D 弱转强”或“B 平台突破”时，不能仅看开盘涨幅。
> 新增量化指标：竞价匹配成交量 / 昨日全天成交量。
> 若弱转强开盘涨幅 ≥ +3%，但竞价成交量 < 昨日全天成交量的 **1.5%**（甚至不足 1%），视为“虚高无量诱多”，**直接拦截买入信号**。（竞价拦截阈值由 V1.4 的 2% 下调至 **1.5%**，更适合激进追入；但依然拦截无量诱多。）

## E 情绪冰点反转
情绪接近冰点 + 跌停减少 + 核心止跌 + 主线重新出现。
仅 **ICE→REPAIR** 阶段小仓试错（E ≤10%），确认后快速增加。
...
- 竞价无量诱多：弱转强/突破开盘涨幅达标但竞价量 < 昨日量 **1.5%**（V1.3；V1.5 阈值 2%→1.5%）
...
单笔最大账户风险（激进模式，较 V1.4 上调上限）：
正常（STRONG/NEUTRAL）：**0.8%～1.5%**（STRONG）／**0.5%～1.0%**（NEUTRAL）
震荡/弱势（WEAK）：**0.3%～0.6%**
EXTREME：**禁新开仓，无风险预算**
...
**仓位提升的硬原则**：仓位只能由“账户允许亏损金额 ÷ 买入价到止损价%”反推，**禁止因“看起来很确定”随意放大**；高确定性只是可以用到更高上限，不是可以超算出的仓位。
...
> **激进模式仓位上限（V1.5，较 V1.4 提升）**；决定仓位依托的是**总仓上限与单笔风险预算**以及**“允许亏损÷买入价到止损价%”反推**，不是靠感觉。

STRONG：总仓 **85%～95%**（V1.4:70%～80% → 提升）
单票：核心 **25%～30%**／普通 **15%～20%**
单笔风险预算：**0.8%～1.5%**

NEUTRAL：总仓 **50%～65%**（V1.4:40%～50% → 提升）
单票：核心 **15%～20%**／普通 **10%～15%**
单笔风险预算：**0.5%～1.0%**

WEAK：总仓 **25%～35%**（V1.4:最大20% → 提升）
试错/核心：**8%～12%**
单笔风险预算：**0.3%～0.6%**

EXTREME：总仓 **0%～5%**，**禁新开仓**，持仓只减不增。

单票上限：试错 **8%～12%**／普通 **15%～20%**／核心 **25%～30%**（V1.4: 试错2%～5%/普通5%～10%/核心10%～20% → 提升）。单票原则上不超过 **30%**（核心）。

同一题材/高度相关股票视为一个风险篮子。
**单一高相关主题原则上不超过 50%**。

⚠️ 仓位提升不改变：仍需由“允许亏损÷买入价到止损价%”反推；`#33.1` 的买入分级、`#27` 候选池、`#44` 许可矩阵不得被放大仓位绕过。
...
情绪止损：市场进入明显退潮。
强弱止损：买入后明显弱于板块核心。
时间止损（量化，V1.5）：**买入后 1～2 个交易日未按预期运行**（如价格未站上买入价上方一定幅度【≥2%】、或相对所属板块持续走弱）→ **降仓或退出**。V1.4 的“2~3 天”已量化收紧为“1~2 个交易日”。

> ⚠️ **禁止越跌越补**：任何情况下都不得用“补仓摊低成本”对抗浮亏；越跌越补是红线（跌破止损即按计划退出，而非加仓）。
...
禁止机械固定“+5%必卖、+10%必卖”的傻瓜式止盈。改为结构化止盈：

**1. 达第一目标（计划收益的 50%～70%）→ 减仓 30%～50% 锁利。**
先兑现部分利润，剩余仓位继续跟踪，而不是一次性卖到天花板或死拿到回落。

**2. 剩余仓位用下面的“结构/强弱”跟踪，而不是只看绝对涨幅：**
- **结构支撑**：是否仍站在关键支撑/均线上方；跌破关键支撑 → 减/清。
- **相对龙头强弱**：是否仍强于/同步所属板块龙头；跑输龙头 → 警惕。
- **板块强弱**：所属板块是否仍收在相对强势；板块转弱 → 退出。

**3. 立刻清仓信号（任一命中，立即清仓，不犹豫）：**
- 龙头明显退潮，或龙头炸板无力回封。
- 板块整体退潮。
- 个股放巨量滞涨，且相对板块转弱。
- 情绪进入 DISTRIBUTION。
...
**4. 达到交易模型目标 → 分批止盈。**
**禁止到了目标不走、幻想更高**；达模型目标后至少减半兑现，剩余按结构/强弱跟踪。

> 激进模式不改变上述止盈纪律：**敢拿住强势的那部分，但一到目标/触发信号就果断分批了结**，不因“短线敢打”而把止盈变成贪心。
...
低于2:1 的例外（V1.5）：
- 除非属于**高确定性特殊机会**：**核心龙 + 主线高潮 + 明确买点**三者同时成立，允许 **≥1.5:1**。
- **必须写清理由，并在交易计划/记录中标注“特殊机会”**，否则仍按 ≥2:1 执行。
- “特殊机会”不能叠加用于延长止损或放大仓位——止损与仓位仍受 `#13`/`#14` 约束。
...
Cron 12：15:10 收盘数据
- 更新最终行情、涨停、跌停、连板、炸板、成交额、板块强度（用 mx-data / mx-search 取数，写入本 SKILL 的 `data/` 候选池与当日快照）。
- 更新 A股短线模拟账户的持仓、资金、订单、成交和交易记录（用 sim_account.py 现成命令）。

**Cron 12 数据读取铁律（强制）：**
- ✅ 读账户状态/持仓/订单/成交/资金 → 一律用模拟账户现成命令：`python3 {{OPENCLAW_WORKSPACE}}/A股短线交易系统/sim/sim_account.py status|positions|orders|fills|cashflow`；日结用 `python3 {{OPENCLAW_WORKSPACE}}/A股短线交易系统/sim/sim_account.py settle --closes <code:price,code:price>`（closes 为收盘价，按实际持仓填写）
- ✅ 读/写短线候选池（watchlist） → 本地文件绝对路径：`{{OPENCLAW_WORKSPACE}}/A股短线交易系统/data/watchlist.json`（用 `read` 读、`write`/`edit` 写，**不用** `cat` 覆盖重写）
- ✅ 读当日快照 → `cat {{OPENCLAW_WORKSPACE}}/A股短线交易系统/sim/data/snapshots/YYYY-MM-DD.json`
- ❌ **禁止** `show` 命令（Linux 无此命令）；**禁止**相对路径 `data/...`（不存在扁平 data/ 目录）；**禁止**臆造 `market.json` 路径（它由 sim_account.py 内部管理，为只读 `{}`，不是 SKILL 数据源）。
- ❌ **禁止** `search` 命令。列目录用 `ls`，读文件用 `read`/`cat`，写文件用 `write`/`edit`。
- 收盘数据不得臆造文件路径；读不到的字段标 UNKNOWN，不得编造。
...
S级：
**3～5只**（V1.4: 1~3 → 放宽；激进模式下可多备几只真正准备交易的）。
真正准备交易。
...
**每天收盘必须清理候选池，禁止无限扩容**（放宽数量 ≠ 保留垃圾股，S 级当天清理由持仓/强度决定去留）。
...
- 风险收益比优秀（盈亏比 ≥ 2，且止损清晰）。
  （#16 例外：核心龙+主线高潮+明确买点的特殊机会，允许 ≥1.5:1，但须书面注明“特殊机会”。）
- 可不打分，但所有分层条件必须逐项确认。
...
以下为**计划仓位强度系数**，实际以现有 `#13 仓位` 规则的单股上限（V1.5：单票试错 8%–12%、普通 15%–20%、核心 25%–30%、核心单票 ≤30%）为基准计算，**不得覆盖现有单股最大仓位**：
...
16. **激进短线模式边界（V1.5）**：敢打前排/敢追核心龙/敢快速加仓，但先确认市场还能赚钱、永远有止损/失效条件、不越跌越补、不硬抗退潮；EXTREME/DISTRIBUTION/炸板率>35%/最高板闪崩 立刻收手；Tool→Evidence→Decision 闭环不可省。
