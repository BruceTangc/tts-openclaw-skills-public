...
# 多策略组合（热号+尾数过滤，各策略权重逐号相乘融合，前后区独立）
cd ~/.openclaw/workspace-jarvis/skills/dlt-simulator/scripts && python3 generator.py --count 10 --strategy "hot+tail_filter"
```
...
| `adjacent_filter` | 邻号过滤策略：压制「与最近一期（data[0]）开奖号码相差 1 的号码（邻号 m±1）」权重到 0.5（体现邻号延续概率低规律），非邻号按 balanced 基础权重（前区 hot/miss>15/rising 加成，后区 hot 加成）；前后区独立统计，空数据退化全 1.0，越界号码忽略 |
