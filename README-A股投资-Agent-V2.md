...
## 最终运行目标
V2 的目标不是做一个只研究不交易的系统，也不是追求零亏损。Agent 在确认使用妙想模拟账户后，应自主分析真实 A 股市场、自主使用妙想等能力发现和验证机会、自主进行模拟资本配置和交易、持续管理持仓，并以真实市场产生的模拟盈亏作为反馈之一进行复盘学习。允许有依据、可控制、可复盘的模拟亏损和试错；禁止无依据地为了交易而交易。长期目标是通过反复的 `发现→研究→决策→模拟执行→跟踪→复盘→学习→再验证` 提高可验证的风险调整后盈利能力。
